...
$a_sql .= " ORDER BY model ASC LIMIT {$limit} OFFSET {$offset_a}";
...
<head>
  ...
  <style>
    ...
    .pagination .disabled { margin-right: 8px; color: #aaa; cursor: default; text-decoration: none; }
    .modal { display: none; position: fixed; top: 10%; left: 10%; width: 80%; max-height: 80%; overflow: auto; background: #fff; padding: 20px; border: 1px solid #ccc; z-index: 1000; }
...
<body>
  ...
  <div class="pagination">
    <?php if ($page_s > 1): ?>
      <a href="?page_s=<?= $page_s - 1 ?>">← Prev</a>
    <?php else: ?>
      <span class="disabled">← Prev</span>
    <?php endif; ?>
    <span>Page <?= $page_s ?></span>
    <?php if (count($slip_summary) === $limit): ?>
  ...
  <form method="get">
    ...
    <label>Office: <select name="b_off">
      ...
      <?php foreach (array_unique(array_column($borrowed, 'office_borrower')) as $off): ?>
  ...
  <table>
    <tr>
      <th>Type</th><th>Brand</th><th>Model</th><th>Serial</th>
      <th>Date Borrowed</th><th>Due Date</th><th>Purpose</th><th>Borrower</th><th>Office</th>
    </tr>
  ...
  <div class="pagination">
    <?php if ($page_b > 1): ?>
      <a href="?page_s=<?= $page_s ?>&page_b=<?= $page_b - 1 ?>&b_type=<?= urlencode($b_type) ?>&b_from=<?= urlencode($b_from) ?>&b_to=<?= urlencode($b_to) ?>&b_off=<?= urlencode($b_off) ?>">← Prev</a>
    <?php else: ?>
      <span class="disabled">← Prev</span>
    <?php endif; ?>
    <span>Page <?= $page_b ?></span>
    <?php if (count($borrowed) === $limit): ?>
      <a href="?page_s=<?= $page_s ?>&page_b=<?= $page_b + 1 ?>&b_type=<?= urlencode($b_type) ?>&b_from=<?= urlencode($b_from) ?>&b_to=<?= urlencode($b_to) ?>&b_off=<?= urlencode($b_off) ?>">Next →</a>
    <?php else: ?>
      <span class="disabled">Next →</span>
    <?php endif; ?>
  </div>
  ...
  <table>
    <tr>
      <th>Type</th><th>Brand</th><th>Model</th><th>Locator</th><th>Serial</th><th>Description</th>
    </tr>
  ...
  <div class="pagination">
    <?php if ($page_a > 1): ?>
      <a href="?page_s=<?= $page_s ?>&page_a=<?= $page_a - 1 ?>&a_type=<?= urlencode($a_type) ?>&a_brand=<?= urlencode($a_brand) ?>&a_model=<?= urlencode($a_model) ?>">← Prev</a>
    <?php else: ?>
      <span class="disabled">← Prev</span>
    <?php endif; ?>
    <span>Page <?= $page_a ?></span>
    <?php if (count($available) === $limit): ?>
      <a href="?page_s=<?= $page_s ?>&page_a=<?= $page_a + 1 ?>&a_type=<?= urlencode($a_type) ?>&a_brand=<?= urlencode($a_brand) ?>&a_model=<?= urlencode($a_model) ?>">Next →</a>
    <?php else: ?>
      <span class="disabled">Next →</span>
    <?php endif; ?>
  </div>
