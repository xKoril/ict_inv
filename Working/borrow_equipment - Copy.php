...
<head>
  ...
  <style>
    ...
    .filter-form { margin-bottom: 20px; display: flex; gap: 12px; }
    .filter-form label { flex: 1; }
    .filter-form button, .filter-form a.btn { flex: 1; text-decoration: none; text-align:center; background: none; border: none; }
...
<body>
  ...
  <form method="post">
    ...
    <button type="submit" style="margin-top:20px;width:100%;">Confirm Borrow Request</button>
  </form>
  ...
  <script>
    function bulkAdd() {
      ...
      Array.from(avail.querySelectorAll('input[type=checkbox]')).forEach(cb => {
        if (cb.checked) {
          const tr = cb.closest('tr'); cb.checked=false;
          sel.appendChild(tr);
          const eid = tr.dataset.eid;
          const inp = document.createElement('input');
          inp.type='hidden'; inp.name='equipment_ids[]'; inp.value=eid;
    ...
    function bulkRemove() {
      ...
      const sel   = document.getElementById('selBody');
      const hidden = document.getElementById('hiddenInputs');
      Array.from(sel.querySelectorAll('input[type=checkbox]')).forEach(cb => {
        if (cb.checked) {
          const tr = cb.closest('tr'); cb.checked=false;
